...
---------------------------------------------------------------------------------------------------------------------------
4. 출결 관리 및 출결 조회 - 월별 테이블
-- 매일 근태 관리를 기록할 수 있어야 한다(출근 1회, 퇴근 1회)
-- 본인의 출결 현황을 기간별(전체, 월, 일)조회 할 수 있어야 한다.
-- 다른 교육생들의 현황은 조회할 수 없다.
-- 모든 출결 조회는 근태 상화을 구분할 수 있어야 한다(정상, 지각, 조퇴, 외출, 병가, 기타..)
---------------------------------------------------------------------------------------------------------------------------
select substr(att.workon,4,2)||'월' as 월별,s.name as "학생명",att.workon as "입실시간" ,att.workoff as "퇴실시간", att.state as "상태" from tblattendance att
    inner join tblregicourse regi
        on regi.regicourse_seq = att.regicourse_seq
            inner join tblStudent s
                on regi.regicourse_seq = s.student_seq; 


---------------------------------------------------------------------------------------------------------------------------
4-2. 일별 테이블
---------------------------------------------------------------------------------------------------------------------------
select substr(att.workon,7,2)||'일' as 일별,s.name as "학생명",att.workon as "입실시간" ,att.workoff as "퇴실시간", att.state as "상태" from tblattendance att
    inner join tblregicourse regi
        on regi.regicourse_seq = att.regicourse_seq
            inner join tblStudent s
                on regi.regicourse_seq = s.student_seq
                    --where to_char(workon,'YY/MM/DD') = '20/03/16'; 

---------------------------------------------------------------------------------------------------------------------------
4-3. 전체 테이블
---------------------------------------------------------------------------------------------------------------------------
select s.name as "학생명",att.workon as "입실시간" ,att.workoff as "퇴실시간", att.state as "상태" from tblattendance att
    inner join tblregicourse regi
        on regi.regicourse_seq = att.regicourse_seq
            inner join tblStudent s
                on regi.regicourse_seq = s.student_seq; 
